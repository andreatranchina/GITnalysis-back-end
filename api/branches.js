...
const githubCache = new NodeCache({ stdTTL: 3600 }); // 3600 seconds = 1 hour
...
router.get("/:owner/:repo", authenticateUser, async (req, res, next) => {
  try {
    ...
    if (cachedData) {
      ...
      githubCache.set(cacheKey, { branches: sortedBranches }, 3600);
...
router.get("/count/:owner/:repo", authenticateUser, async (req, res, next) => {
  try {
    ...
    if (cachedData) {
      ...
      githubCache.set(cacheKey, { branches: all_branches.length }, 3600);
  ...
  async (req, res, next) => {
    try {
      ...
      if (cachedData) {
        ...
        githubCache.set(cacheKey, { branch: branchResponse }, 3600);
